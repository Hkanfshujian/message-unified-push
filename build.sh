...
clean_build() {
    ...
    # 清理后端构建文件
    if [ -f "ops-message-unified-push" ]; then
        rm -f ops-message-unified-push
    ...
    if [ -f "ops-message-unified-push.exe" ]; then
        rm -f ops-message-unified-push.exe
...
build_backend() {
    ...
    case "$OS" in
        Linux*)
            ...
            CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "${LDFLAGS}" -o ops-message-unified-push main.go
            ;;
        Darwin*)
            print_info "检测到 macOS 系统，构建 macOS 版本..."
            go build -ldflags "${LDFLAGS}" -o ops-message-unified-push main.go
            ;;
        MINGW*|MSYS*|CYGWIN*)
            print_info "检测到 Windows 系统，构建 Windows 版本..."
            go build -ldflags "${LDFLAGS}" -o ops-message-unified-push.exe main.go
            ;;
        *)
    ...
    # 检查构建结果
    if [ -f "ops-message-unified-push" ] || [ -f "ops-message-unified-push.exe" ]; then
        print_success "后端构建完成！"
        
        # 显示文件信息
        if [ -f "ops-message-unified-push" ]; then
            FILE_SIZE=$(du -h ops-message-unified-push | cut -f1)
            print_info "可执行文件: ops-message-unified-push (${FILE_SIZE})"
        fi
        
        if [ -f "ops-message-unified-push.exe" ]; then
            FILE_SIZE=$(du -h ops-message-unified-push.exe | cut -f1)
            print_info "可执行文件: ops-message-unified-push.exe (${FILE_SIZE})"
        fi
    else
        print_error "后端构建失败！"
        exit 1
...
build_cross_platform() {
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "${LDFLAGS}" -o ${BUILD_DIR}/ops-message-unified-push-linux-amd64 main.go
    
    # 构建 Linux arm64
    print_info "构建 Linux arm64..."
    CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags "${LDFLAGS}" -o ${BUILD_DIR}/ops-message-unified-push-linux-arm64 main.go
    
    # 构建 macOS amd64
    print_info "构建 macOS amd64..."
    CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -ldflags "${LDFLAGS}" -o ${BUILD_DIR}/ops-message-unified-push-darwin-amd64 main.go
    
    # 构建 macOS arm64 (Apple Silicon)
    print_info "构建 macOS arm64..."
    CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags "${LDFLAGS}" -o ${BUILD_DIR}/ops-message-unified-push-darwin-arm64 main.go
    
    # 构建 Windows amd64
    print_info "构建 Windows amd64..."
    CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -ldflags "${LDFLAGS}" -o ${BUILD_DIR}/ops-message-unified-push-windows-amd64.exe main.go
...
main() {
    ...
    if [ -f "ops-message-unified-push.exe" ]; then
        echo ""
        print_info "运行应用:"
        echo "  ops-message-unified-push.exe"
        echo ""
    fi
}

...
::: tip ⭐ 推荐使用
V2 API（模板）是我们推荐且默认的消息发送方式，具有以下优势：
...
V2 API 基于“模板 + 占位符”机制，适合绝大多数通知、告警和自动化消息场景。

- 模板在后台统一维护，调用方只需传 `token + placeholders`
- 支持多实例并行发送（邮件、IM、Webhook 等）
- 支持动态接收者（群发）和多格式内容自动匹配
...
**A:** 新接入场景不建议继续使用 V1。建议统一使用 V2，历史任务按迁移计划逐步切换。
...
- 查看 [API 使用说明](/api/usage) 了解模板发送方式

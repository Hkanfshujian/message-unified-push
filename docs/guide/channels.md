...
ops-message-unified-push 支持多种消息推送渠道，您可以根据业务场景组合多个渠道，实现统一入口、多端分发。

## 快速接入流程（推荐）

1. 在本页选择并配置至少一个推送渠道
2. 在 [消息模板](/guide/template) 中创建模板并绑定渠道
3. 使用 [V2 API](/api/v2) 发送模板消息
4. 在 [定时消息](/guide/scheduled-messages) 中配置定时推送（可选）

## 渠道选型建议

| 场景 | 推荐渠道 |
|------|----------|
| 业务告警/值班通知 | 钉钉、企业微信、飞书、Telegram |
| 用户触达 | 邮件、微信测试公众号 |
| 移动端即时提醒 | Bark、PushMe |
| 系统集成 | 自定义 Webhook |
...
4. **在 ops-message-unified-push 中配置**
   - 渠道名称：自定义名称
...
3. **在 ops-message-unified-push 中配置**
...
| 存档 | 是否存档 (1 或 0) | 否 | `1` |
| 分组 | 推送分组 | 否 | `OpsMessageUnifiedPush` |
| 推送声音 | 推送铃声名称 | 否 | `glass` |
...
2. **在 ops-message-unified-push 中配置**
   - Push Key：填入您的 Push Key
   - 自定义 API 地址：如果使用自建服务，请填写完整的 API 地址（如 `https://push.example.com/`）
   - 其他参数：根据需要填写日期、类型等
...
| 参数 | 说明 | 必填 | 示例 |
|------|------|------|------|
| 渠道名称 | 自定义渠道名称 | 是 | `PushMe通知` |
| Push Key | PushMe Key | 是 | `your_push_key` |
| 自定义 API 地址 | 自建服务地址 | 否 | `https://push.i-i.me/` |
| 日期 | 消息日期 | 否 | `2024-01-01` |
| 类型 | 消息类型 | 否 | `text` |
...
5. **在 ops-message-unified-push 中配置**
...
- 查看 [消息模板](/guide/template) 了解如何创建和绑定渠道模板
- 查看 [定时消息](/guide/scheduled-messages) 了解如何定时发送
- 查看 [V2 API 文档](/api/v2) 了解如何通过模板 API 发送消息

...
::: warning 重要提示
请确保所选的消息模板已配置至少一个发送实例，否则无法发送消息。
:::
...
1. 点击定时消息的"删除"按钮
2. 在弹出的确认对话框中，根据提示输入定时消息名称（支持复制红色高亮的名称）
3. 确认删除操作
4. 定时消息将被删除，不再执行
...
- ✅ 为重要消息配置多个渠道
- ✅ 使用有意义的任务名称
 - ✅ 删除消息模板前，先在"定时消息"中删除所有使用该模板的任务（系统会进行约束检查）
...
**A:** 
定时消息需要关联一个消息模板，定时消息只负责定时触发，实际的消息发送由关联的消息模板完成。模板决定了消息发送到哪些渠道。

## 注意事项

::: warning 重要提示
1. **关联模板配置** - 确保关联的消息模板已配置至少一个发送实例
...
- 查看 [消息模板](/guide/template) 了解如何配置消息模板
- 查看 [V2 API 文档](/api/v2) 了解消息发送机制

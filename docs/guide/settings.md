...
系统设置页面允许您配置 ops-message-unified-push 的各项参数，包括站点信息、日志清理、密码管理等。
...
| 设置项 | 说明 | 默认值 |
|--------|------|--------|
| 站点标题 | 显示在浏览器标题和页面顶部 | `ops-message-unified-push` |
| 站点标语 | 显示在登录页面的标语 | `消息推送整合平台` |
| 站点图标 | 网站 Logo（仅支持 SVG 文本） | 默认 Logo |
| 主题色 🆕 | 系统主色，支持预置主题和自定义颜色（HEX/RGB） | 默认蓝色 |
| 侧边栏背景色 🆕 | 左侧导航栏背景色，支持自定义和常用颜色收藏 | 深色主题色 |
...
1. 登录管理后台
2. 进入"系统设置" → "站点设置"
3. 填写或修改相关信息（包括主题色和侧边栏背景色）
4. 保存设置
5. 主题色和侧边栏背景色会立即生效；站点标题/标语等与缓存相关的配置，下次登录时生效（如不生效，在登录页面 Ctrl+F5 强制刷新）
...
加解密工具用于在后台方便地进行 **模板ID ↔ 加密 Token** 的转换，以及自动生成符合规则的模板ID。

### 功能概述

- **生成模板ID**：一键生成符合规则的模板ID（`TP` 开头 + 10 位字母数字）
- **模板ID → token**：根据模板ID生成 V2 接口所需的加密 Token
- **token → 模板ID**：根据已有 Token 反查对应的模板ID，便于排查问题

### 使用场景

- ✅ 在对接 V2 API 时，快速生成模板ID和 Token  
- ✅ 排查线上调用问题时，从 Token 反推具体模板  
- ✅ 与外部系统进行模板ID约定时，生成规范的ID

### 配置步骤

1. 登录管理后台
2. 进入"系统设置" → "加解密工具"
3. 使用"生成模板ID"自动生成 ID（可复制到模板管理中使用）
4. 在"模板ID → token"区域输入模板ID，点击"生成 token"
5. 在"token → 模板ID"区域输入 Token，点击"解析模板ID"
...
| 信息项 | 说明 |
|--------|------|
| 系统版本 | ops-message-unified-push 版本号 |
| 构建时间 | 系统构建时间 |
| 内存使用 | 当前内存使用情况 |
| 运行时间 | 系统已运行时长 |

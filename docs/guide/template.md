...
::: tip ⭐ 作者推荐
消息模板是 ops-message-unified-push 的核心功能，也是我们**强烈推荐**的使用方式。相比传统的任务发送（V1 API），模板方式具有以下显著优势：
...
- **模板名称** - 模板的唯一标识名称
- **模板ID（可选）** 🆕 - 模板的系统ID  
  - 不填写时，系统会自动生成形如 `TPxxxxxxxxxx` 的ID（`TP` + 10位字母数字）  
  - 如需与外部系统做显式绑定，可以在"加解密工具"中生成符合规则的ID后填写  
  - ID 一旦创建，不建议修改，避免影响定时消息等外部关联
- **模板描述** - 模板的用途说明（可选）
- **状态** - 启用/禁用
...
V2 接口通过 **模板 Token** 来识别要使用的模板。Token 与模板ID是一一对应的：

- 模板ID固定时，对应的 Token 也是固定的（可重复生成）
- Token 只是对模板ID做了简单加密和编码，方便对外使用

获取 Token 的方式：

- 在管理后台的 **系统设置 → 加解密工具** 中，通过"模板ID → token"进行转换  
- 或在模板列表中打开"接口"示例，查看示例中使用的 Token
...
1. 确认模板ID（系统自动生成或手动指定）
2. 使用"加解密工具"将模板ID转为 Token
3. 准备占位符数据
4. 调用 V2 API 发送

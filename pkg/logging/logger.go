...
func Setup() {
	...
	logrus.SetOutput(&filteringWriter{
		out: os.Stdout,
	})
...
func (w *filteringWriter) Write(p []byte) (n int, err error) {
	line := string(p)
	if shouldDropRocketMQNoise(line) {
		// 吞掉噪音日志，但返回“已写入”避免影响调用方
		return len(p), nil
	}
	return w.out.Write(p)
}

func shouldDropRocketMQNoise(line string) bool {
	noisePatterns := []string{
		"update offset to broker success",
		"push consumer quit pullMessage for dropped queue",
		"MessageQueue do balance done",
	}
